...
contract TimesheetContract {
    ...
    event TimesheetAdded(uint256 timesheetTimestamp, address worker, string description);
    event TimesheetWithWorkerNameAdded(uint256 timesheetTimestamp , string workerName , string description);
    ...
    function addTimesheetWithAccount( string memory _description) public {
        ...
        emit TimesheetAdded(newTimesheet.timesheetTimestamp, newTimesheet.workerAddress, newTimesheet.description);
    }

   
    function addTimesheetWithWorkerName( string memory _workerName, string memory _description) public {
        TimesheetWithWorkerName memory newTimesheetWithWorkerName = TimesheetWithWorkerName({
            timesheetTimestamp: block.timestamp,
        ...
        emit TimesheetWithWorkerNameAdded(newTimesheetWithWorkerName.timesheetTimestamp, newTimesheetWithWorkerName.workerName, newTimesheetWithWorkerName.description);
    }
     ...
     */
    function getTimesheet(uint256 index) public view returns (uint256, address, string memory) {
     ...
     * @param index Index of the timesheet in the array.
     */
    function getTimesheetWithWorkerName(uint256 index) public view returns (uint256, string memory, string memory) {
        require(index < timesheetsWithWorkerName.length, "Index out of bounds.");
